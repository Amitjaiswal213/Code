//Q1) Write a program in C language to Cluster a set of two dimensional Data points , using K-Means Clustering Algorithm. Consider fixed initial centroids, and value of k =2.
...
int main()
{
 ...
 prev1.x = -1;
 prev1.y = -1;
 while (max > 0 && (prev1.x != cent0.x && prev1.y != cent0.y))//until the iterations are completed 
//or previous value of centroid is not equal to current value
 {
 for (int i = 0; i < n; i++)//all data items are taken
 {
 if (dist(data[i].x, data[i].y, cent0.x, cent0.y) > dist(data[i].x, data[i].y, cent1.x, cent1.y))// if dist 
//of a point from its centroid is greater than other centroid
 {
