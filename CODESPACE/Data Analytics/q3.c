//3) Write a program in C language to cluster a set of 2 dimensional Data points, using K-Means Clustering Algorithm. Assume the number of clusters to be entered by the user. 
//Execute it for three execution instances taking RANDOM initial centroids, but same k for 
//each of the instances. Find out the number of iterations, the sum squared error in each 
//instance, and print the instance for which you are achieving least error [also display the 
//centroids generated here].
...
int main()
{
 ...
 printf("\nNo of iterations for instance %d is %d", ind + 1, iter);
 ind++;
 }
 printf("\nwe got minimum error in the %d instance with error %d\n", instance, minm);
 return 0;
}